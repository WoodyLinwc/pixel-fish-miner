...
import { GameState, FishType, Language, AchievementCategory, WeatherType } from './types';
import { INITIAL_GAME_STATE, UPGRADES, ACHIEVEMENTS, FISH_TYPES, POWERUPS, COSTUMES, PETS } from './constants';
...
const App: React.FC = () => {
  ...
  const [language, setLanguage] = useState<Language>(() => {
    ...
    return (savedLang === 'en' || savedLang === 'es' || savedLang === 'zh') ? savedLang : 'en';
  });
  ...
  useEffect(() => {
    const timer = setInterval(() => {
        setGameState(prev => {
            ...
            if (Object.keys(prev.activePowerups).length > 0 || prev.weatherExpiration) {
                needsUpdate = true;
            }
    ...
    const comboTimer = setInterval(() => {
        setGameState(prev => {
            if (prev.currentCombo > 0) {
                ...
                if (!isAutoPaused && !isStoreOpen && !isBagOpen && !isAchievementsOpen && timeSinceLastCatch > 10000) {
  ...
  useEffect(() => {
    const weatherTimer = setInterval(() => {
        setGameState(prev => {
            ...
            if (isAutoPaused || isStoreOpen || isBagOpen || isAchievementsOpen) return prev;
            ...
            // If CLEAR, 5% chance to change to a random weather (excluding Rainbow)
                ...
                if (Math.random() < 0.05) {
                    ...
                    else if (r < 0.50) newWeather = WeatherType.SNOW;
  ...
  const checkAchievements = (newState: GameState) => {
    ...
    ACHIEVEMENTS.forEach(ach => {
        ...
        let achieved = false;
        switch(ach.category) {
            case AchievementCategory.FISH: achieved = totalFish >= ach.threshold; break;
            case AchievementCategory.TRASH: achieved = totalTrash >= ach.threshold; break;
            case AchievementCategory.MYSTERY: achieved = totalMystery >= ach.threshold; break;
            case AchievementCategory.MONEY: achieved = totalMoney >= ach.threshold; break;
            case AchievementCategory.WEATHER: achieved = totalWeatherFish >= ach.threshold; break;
            case AchievementCategory.COMBO: achieved = maxCombo >= ach.threshold; break;
            case AchievementCategory.NARWHAL: achieved = (newState.fishCaught['narwhal'] || 0) >= ach.threshold; break;
        }
  ...
  const handleBuyUpgrade = (upgradeId: string) => {
    setGameState(prev => {
        ...
        if (upgradeId === 'clawStrength') currentLevel = prev.clawStrengthLevel || 1;
        if (upgradeId === 'fishDensity') currentLevel = prev.fishDensityLevel || 1;

        const cost = Math.floor(upg.baseCost * Math.pow(upg.costMultiplier, currentLevel - 1));
  ...
  const handleDowngradeUpgrade = (upgradeId: string) => {
    setGameState(prev => {
        ...
        if (currentLevel > 1) {
            ...
            const refund = Math.floor(upg.baseCost * Math.pow(upg.costMultiplier, currentLevel - 2));
  ...
  const handleBuyPowerup = (powerupId: string) => {
     ...
     const powerupKey = Object.keys(POWERUPS).find(key => POWERUPS[key].id === powerupId);
     ...
     setGameState(prev => {
         ...
         if (prev.money >= cost) {
             return {
                 ...
                 purchasedPowerups: hasBoughtBefore ? prev.purchasedPowerups : [...prev.purchasedPowerups, powerupId]
             };
         }
         return prev;
     });
  ...
  const handleActivatePowerup = (powerupId: string) => {
      ...
      if (powerupId === 'magicConch') {
          const weathers = [WeatherType.RAIN, WeatherType.SNOW, WeatherType.WIND, WeatherType.FOG];
  ...
  const handleBuyCostume = (costumeId: string) => {
     ...
     setGameState(prev => {
         if (prev.money >= costume.cost && !prev.unlockedCostumes.includes(costumeId)) {
             return {
                 ...prev,
  ...
  const handleApplyPromoCode = (code: string): { success: boolean; message: string } => {
    ...
    // Weather Codes
    if (cleanCode === 'rain') {
        setGameState(prev => ({...prev, weather: WeatherType.RAIN, weatherExpiration: undefined}));
        return { success: true, message: t.weatherRain };
    }
    if (cleanCode === 'snow') {
        setGameState(prev => ({...prev, weather: WeatherType.SNOW, weatherExpiration: undefined}));
        return { success: true, message: t.weatherSnow };
    }
    if (cleanCode === 'wind') {
        setGameState(prev => ({...prev, weather: WeatherType.WIND, weatherExpiration: undefined}));
        return { success: true, message: t.weatherWind };
    }
    if (cleanCode === 'fog') {
        setGameState(prev => ({...prev, weather: WeatherType.FOG, weatherExpiration: undefined}));
        return { success: true, message: t.weatherFog };
    }
    
    // Rainbow Code - ONE TIME USE
    if (cleanCode === 'rainbow') {
        if (gameState.usedPromoCodes && gameState.usedPromoCodes.includes('rainbow')) {
    ...
    if (cleanCode === 'normal') {
        setGameState(prev => ({...prev, weather: WeatherType.CLEAR, weatherExpiration: undefined}));
  ...
  const clawThrowSpeedMultiplier = 1 + ((gameState.clawStrengthLevel || 1) - 1) * 0.15;
  const t = TRANSLATIONS[language];

  // Get current achievement to show
  const currentAchievementId = achievementQueue[0];
  const currentAchievement = currentAchievementId ? ACHIEVEMENTS.find(a => a.id === currentAchievementId) : null;
      ...
      <div className="w-full max-w-[1024px] flex flex-col shadow-[0_0_20px_rgba(0,0,0,0.5)]">
        ...
        <div className="relative border-x-8 border-[#5d4037] bg-[#5d4037]">
            <GameCanvas 
                ...
                paused={isStoreOpen || isBagOpen || isAchievementsOpen || isAutoPaused}
            ...
            {isAutoPaused && (
                ...
                >
                    <div className="bg-[#5d4037] p-8 border-4 border-[#8d6e63] rounded-lg shadow-[0_0_20px_rgba(0,0,0,0.8)] text-center transform hover:scale-105 transition-transform duration-200">
                        <h1 className="text-4xl text-[#ffeb3b] mb-4 drop-shadow-[4px_4px_0_rgba(0,0,0,1)] tracking-widest">{t.paused}</h1>
                        <div className="flex items-center justify-center gap-2 text-white animate-pulse">
                            <Play size={20} fill="white" />
                            <span className="text-sm font-bold uppercase">{t.resume}</span>
                        </div>
                    </div>
                </div>
            )}
