...
import { drawOldBoot, drawRustyCan, drawPlasticBottle, drawStraw } from './fish/trash';
import { drawShell, drawSeaCucumber, drawMysteryBag, drawSupplyBox, drawCoral } from './fish/staticItems';
import { drawSardine, drawHerring, drawSmallYellowCroaker, drawMackerel, drawCod, drawBoxfish, drawPomfret } from './fish/commonFish';
import { drawClownfish, drawSquid, drawSeaBass, drawRedSnapper, drawSalmon, drawTuna, drawNeedlefish } from './fish/uncommonFish';
import { drawLargeYellowCroaker, drawTurbot, drawRibbonfish, drawGiantGrouper, drawAnglerfish, drawWolffish, drawCrab, drawElectricJelly } from './fish/rareFish';
import { drawWhale, drawNarwhal } from './fish/legendaryFish';
import { drawThunderEel, drawIceFin, drawWindRay, drawSeaTurtle } from './fish/weatherFish';

export const drawEntity = (ctx: CanvasRenderingContext2D, entity: EntityFish, rotation: number = 0, time: number = 0) => {
    ctx.save();
    ...
    else if (type.id === 'small_yellow_croaker') drawSmallYellowCroaker(ctx, w, h);
    ...
    else if (type.id === 'large_yellow_croaker') drawLargeYellowCroaker(ctx, w, h);
