...
const AchievementsModal: React.FC<AchievementsModalProps> = ({ isOpen, onClose, gameState, language }) => {
  ...
  const getProgress = (category: AchievementCategory) => {
    switch(category) {
        case AchievementCategory.MONEY: return totalMoney;
        case AchievementCategory.FISH: return totalFish;
        case AchievementCategory.TRASH: return totalTrash;
        case AchievementCategory.MYSTERY: return totalMystery;
        case AchievementCategory.COMBO: return maxCombo;
        case AchievementCategory.WEATHER: return totalWeatherFish;
        case AchievementCategory.NARWHAL: return gameState.fishCaught['narwhal'] || 0;
        default: return 0;
    }
  };
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-[2px] p-4">
      {/* Wood Frame */}
      <div className="bg-[#e6c288] border-[6px] border-[#8d5524] rounded-lg w-full max-w-2xl shadow-[0_10px_20px_rgba(0,0,0,0.5)] relative animate-fade-in p-1">
        {/* Inner Border */}
        <div className="border-2 border-[#c68c53] p-4 rounded h-full bg-[#e6c288] max-h-[80vh] flex flex-col">
            <button 
              onClick={onClose}
            ...
            <div className="overflow-y-auto pr-2 custom-scrollbar grid grid-cols-1 gap-3">
                {ACHIEVEMENTS.map((ach) => {
                    ...
                    const percent = Math.min(100, Math.floor((currentProgress / ach.threshold) * 100));
                    
                    // Format description
                    let desc = t.achievementDesc[ach.category].replace('{0}', ach.threshold.toString());
                    ...
                    return (
                        ...
                        >
                            <div className={`w-12 h-12 rounded border flex items-center justify-center text-2xl ${
                                isUnlocked ? 'bg-[#fff59d] border-[#fbc02d]' : 'bg-[#bcaaa4] border-[#8d6e63] grayscale'
                            }`}>
                                {ach.icon}
                            </div>
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-center mb-1">
                                    <h4 className={`font-bold text-xs sm:text-sm ${isUnlocked ? 'text-[#f57f17]' : 'text-[#5d4037]'}`}>
                                        {desc}
                                    </h4>
                                    {isUnlocked ? (
                                ...
                                <div className="w-full h-3 bg-[#efebe9] rounded-full border border-[#d7ccc8] overflow-hidden relative">
                                    <div 
                                        className={`h-full transition-all duration-500 ${isUnlocked ? 'bg-[#fbc02d]' : 'bg-[#8d6e63]'}`} 
                                        style={{ width: `${percent}%` }}
                                    />
