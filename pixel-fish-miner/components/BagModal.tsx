...
const FishIcon: React.FC<{ type: FishType }> = ({ type }) => {
  ...
  useEffect(() => {
    ...
    const scale = Math.min(availableWidth / type.width, availableHeight / type.height);
...
const BagModal: React.FC<BagModalProps> = ({ isOpen, onClose, gameState, language }) => {
            ...
            <div className="overflow-y-auto pr-2 custom-scrollbar grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
                {FISH_TYPES.map((fish) => {
                    ...
                    if (!isUnlocked && count === 0) {
                        // Render locked state
                        return (
                            <div key={fish.id} className="bg-[#d7ccc8] opacity-50 p-3 rounded border-2 border-[#a1887f] flex items-center gap-4 select-none">
                                <div className="w-12 h-12 bg-[#bcaaa4] rounded flex items-center justify-center text-[#8d6e63] font-bold text-xl">?</div>
                    ...
                    return (
                        <div key={fish.id} className="bg-[#fff3e0] p-3 rounded border-2 border-[#a1887f] shadow-sm flex items-center gap-4 relative">
                            <div className="w-12 h-12 flex items-center justify-center relative overflow-hidden shrink-0">
                                {/* Actual sprite rendering */}
                                <FishIcon type={fish} />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h4 className={`font-bold text-sm truncate ${fish.isTrash ? 'text-[#795548]' : 'text-[#3e2723]'}`}>
                                    {fishName}
                                </h4>
